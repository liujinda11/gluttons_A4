...
class MainMenu(cocos.menu.Menu):
    ...
    def select_mode(self, mode_name):
        ...
        if mode_name == 'Classic Mode':
            classic_mode_scene = cocos.scene.Scene(ClassicMode(self.user_type))
            cocos.director.director.replace(classic_mode_scene)
        elif mode_name == 'Unlimited Firepower Mode':
            unlimited_firepower_mode_scene = cocos.scene.Scene(UnlimitedFirepowerMode(self.user_type))
    ...
    def toggle_pause(self):
        if self.paused:
            ...
            if self.pause_menu is None:
                self.paused = True
                self.pause_menu = PauseMenu(self.user_type)  # 确保PauseMenu是以正确的方式创建的!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def on_key_press(self, key, modifiers):
        if not self.gameover.visible:
            if key == pyglet.window.key.SPACE:
                ...
                homepage_scene = HomepageScene(self.user_type)
                cocos.director.director.replace(homepage_scene)
                return True
        else:
...
class PauseMenu(Layer):  # 暂停面板部分只解决鼠标点击的问题，不解决按键的问题
    ...
    def on_main_menu(self):
        print(' # menu go back...\n\n\n')
        main_menu_scene = cocos.scene.Scene(MainMenu(self.user_type))

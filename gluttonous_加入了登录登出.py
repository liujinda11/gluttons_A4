...
def check_credentials(username, password):
    ...
    return True  # 暂时返回True用于示例
    #return match_user_information(username, password)

def register_new_user(username, password):
    # 这里的实现应该将新用户添加到数据库或其他存储机制
...
class LoginLayer(Layer):
    ...
    def build_menu(self):
        # 创建登录界面标签
        self.add(Label('Login', font_size=32, anchor_x='center', anchor_y='center'), z=1)
        
        # 创建菜单项
        login_item = MenuItem('Login', self.on_login)
        register_item = MenuItem('Register', self.on_register)
        guest_item = MenuItem('Play as Guest', self.on_play_as_guest)

        # 创建菜单
        self.menu = Menu()
        self.menu.create_menu([login_item, register_item, guest_item], zoom_in(), zoom_out())
        self.add(self.menu)

    def on_login(self):
        if check_credentials(self.username, self.password):
            self.parent.switch_to_menu(logged_in=True)
        else:
            print("Invalid credentials!")

    def on_register(self):
        register_new_user(self.username, self.password)
        print("Registration successful!")
        self.parent.switch_to_menu(logged_in=True)

    def on_play_as_guest(self):
        self.parent.switch_to_menu(logged_in=False)


class MainScene(cocos.scene.Scene):
    def __init__(self):
        super(MainScene, self).__init__()
        # 一开始将LoginLayer添加到场景中
        self.login_layer = LoginLayer() 
        self.add(self.login_layer, z=1)

    def switch_to_menu(self, logged_in=False):
        # 移除LoginLayer并添加MainMenu层
        self.remove(self.login_layer)
        main_menu_layer = MainMenu(logged_in=logged_in)
        self.add(main_menu_layer, z=0)
        if not logged_in:
            print("Playing as guest. Some features may be disabled.")
        else:
            print("Logged in. All features are available.")
...
class MainMenu(cocos.menu.Menu):
    def __init__(self, logged_in=False):
        ...
        main_menu_items = [cocos.menu.MenuItem('Classic Mode', self.on_classic_mode),
                           ...
                           cocos.menu.MenuItem('Log out', self.on_logout),
                           cocos.menu.MenuItem('Exit the game', self.on_exit_the_game)]
        # 没有字体等参数
        # custom_menu = CustomMenu(items)
        # 创建选项
        if self.logged_in:
            main_menu_items.insert(3, cocos.menu.MenuItem('Ranking list', self.on_ranking_list))
            main_menu_items.insert(4, cocos.menu.MenuItem('Shop', self.on_shop))
    ...
    def on_logout(self):
        # 登出账号,返回登录界面
        self.selected_item = 'Logout'
        print("Selected item:", self.selected_item)
        login_scene = cocos.scene.Scene(LoginLayer())

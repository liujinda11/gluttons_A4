...
from cocos.menu import Menu, MenuItem, zoom_in, zoom_out, EntryMenuItem
...
import pygame
import define
from account import insert_account,update_score,match_user_information,get_ranking,change_cust,change_music,change_volume,change_evolume,current_state,change_cmode, get_ranking
...
class LoginEntryLayer(Layer):
    def __init__(self, entry_type):
        ...
        # 添加标题
        title_label = Label('Login' if entry_type == 'login' else 'Register',
...
class LoginEntryMenu(Menu):
    ...
    def on_login_pressed(self):
        ...
        # 检查数据库中是否有对应的账号密码
        if account.match_user_information(username, password):
            # 如果有，则转到Main menu
...
class HomepageScene(Scene):
    def __init__(self):
        ...
        # 创建标题标签
        title_label = Label('Gluttonous Snake',
                            font_name='Times New Roman',
                            font_size=72,
                            anchor_x='center',
                            anchor_y='center')
        title_label.position = (window_width * 0.5, window_height * 0.8)
        self.add(title_label, z=0)

        main_menu_layer = MainMenu()
        self.add(main_menu_layer, z=1)
...
class MainMenu(cocos.menu.Menu):
    def __init__(self, user_type):
        ...
        # 根据用户类型显示或隐藏某些菜单项
        if user_type == 'guest':
            filtered_menu_items = [item for item in main_menu_items if item.label != 'Ranking List' and item.label != 'Shop']
        else:
            filtered_menu_items = main_menu_items

        self.create_menu(filtered_menu_items)
...
class ClassicMode_guest(cocos.layer.Layer):
    is_event_handler = True
...
class RankingList(Scene):
    ...
    def build_menu(self):
        ...
        for rank in self.ranking_items:
            ...
            username_label = Label(username,position=(director.window.width // 5, director.window.height // 5 * 4 - line * director.window.height // 10),
                                         font_size=32, anchor_x='center', anchor_y='center')
            
            highest_score_label = Label(str(highest_score),position=(director.window.width // 5 * 2,director.window.height // 5 * 4 - line * director.window.height // 10),
                                         font_size=32, anchor_x='center', anchor_y='center')
            ranking_label = Label(str(ranking),position=(director.window.width // 5 * 4, director.window.height // 5 * 4 - line * director.window.height // 10),
                                         font_size=32, anchor_x='center', anchor_y='center')
